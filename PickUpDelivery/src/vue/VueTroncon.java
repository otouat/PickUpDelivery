...
public class VueTroncon {
	...
	/**
	 * Cette méthode permet de dessiner les troncons d'un plan donné
	 * 
	 * @param plan            : on dessine tous les troncons de ce plan
	...
	/**
	 * Cette méthode permet de dessiner les troncons d'une tournée donnée
	 * 
	 * @param plan        : on dessine tous les troncons de ce plan
	 * @param tourneePane : zone où on dessine ces troncons
	 */
	public static void drawTournee(List<Noeud> noeuds, AnchorPane tourneePane) {
		Path path = new Path();
		path.getElements().add(new MoveTo(VueUtils.getNewX(noeuds.get(0).GetLongitude()),
				VueUtils.getNewY(noeuds.get(0).GetLatitude())));
		for (int i = 0; i < noeuds.size() - 1; i++) {
			if (!noeuds.get(i).equals(noeuds.get(i + 1))) {
				drawTournee(noeuds.get(i), noeuds.get(i + 1), tourneePane);
				path.getElements().add(new LineTo(VueUtils.getNewX(noeuds.get(i).GetLongitude()),
						VueUtils.getNewY(noeuds.get(i).GetLatitude())));
			}
		}
		path.getElements().add(new LineTo(VueUtils.getNewX(noeuds.get(noeuds.size() - 1).GetLongitude()),
				VueUtils.getNewY(noeuds.get(noeuds.size() - 1).GetLatitude())));
